...
class Game {
    ...
    renderCharacters() {
        ...
        grid.innerHTML = this.characters.map(char => `
            <div class="character-card" data-id="${char.id}" onclick="game.selectCharacter('${char.id}')">
    ...
    selectCharacter(id) {
        ...
        if (previewAvatar) previewAvatar.textContent = character.avatar || '👤';
        if (previewName) previewName.textContent = character.name;
        if (previewDesc) previewDesc.textContent = character.monologue || character.description || '';
        
        if (previewStats && character.initial_attributes) {
            previewStats.innerHTML = Object.entries(character.initial_attributes)
    ...
    updateDayDisplay() {
        ...
        if (dayNumber) dayNumber.textContent = this.state.progress.currentDay;
        if (timePeriod) timePeriod.textContent = GAME_CONFIG.PERIOD_NAMES[this.state.progress.currentPeriod];
    }
    ...
    updateStatsDisplay() {
        ...
        for (const [key, value] of Object.entries(attrs)) {
            const statValue = document.getElementById(`stat-${key}`);
            ...
            if (barFill) {
                const percentage = this.attributes.getPercentage(key);
    ...
    generateEvent() {
        ...
        if (dialogueText) dialogueText.textContent = event.description || '无事发生...';
        ...
        if (choicesPanel) {
            ...
            choicesPanel.innerHTML = availableOptions.map((option, index) => {
                ...
                return `
                    <button class="choice-btn ${disabledClass}" 
     ...
     */
    showEnding() {
        const ending = this.endings.determineEnding();
        ...
        if (endingSubtitle) endingSubtitle.textContent = ending.description;

        // 最终属性
        if (finalStats) {
            finalStats.innerHTML = Object.entries(ending.finalStats)
                .map(([key, value]) => `
                    <div class="final-stat">
                        <div class="final-stat-icon">${ATTRIBUTE_ICONS[key] || '📊'}</div>
                        <span class="final-stat-name">${ATTRIBUTE_NAMES[key] || key}</span>
                        <span class="final-stat-value">${this.formatAttributeValue(key, value)}</span>
                    </div>
                `).join('');
        }
        ...
        // 假期回顾
        if (endingStory) {
            endingStory.innerHTML = ending.story;
        }
